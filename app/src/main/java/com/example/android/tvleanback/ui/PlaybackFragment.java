...
public class PlaybackFragment extends VideoSupportFragment {
    ...
    private ArrayList<Video> listaAtras;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (!checkIsTelevision()) {
            //Escribe en el firebase
            FirebaseDatabase database = FirebaseDatabase.getInstance();
            DatabaseReference myRef = database.getReference("message");
            myRef.setValue(mVideo);
            ...
            myRef.addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(DataSnapshot dataSnapshot) {
                    Video mVideo2 = dataSnapshot.getValue(Video.class);
                    play(mVideo2);
                }
                @Override
    ...
    private void prepareMediaForPlaying(Uri mediaSourceUri) {
        String source = mediaSourceUri.toString();
        ...
        } else if (source.contains(".m3u8")) {
            // Create a data source factory.
            DataSource.Factory dataSourceFactory = new DefaultHttpDataSourceFactory(Util.getUserAgent(this.getContext(), "app-name"));
            // Create a HLS media source pointing to a playlist uri.
            HlsMediaSource hlsMediaSource = new HlsMediaSource.Factory(dataSourceFactory).createMediaSource(mediaSourceUri);
    ...
    private boolean checkIsTelevision() {
        int uiMode = getActivity().getResources().getConfiguration().uiMode;
    ...
    class PlaylistActionListener implements VideoPlayerGlue.OnActionClickedListener {
        ...
        public void onPrevious() {
            ...
            Video anterior = listaAtras.get(listaAtras.size()- 2);
            //Toast.makeText(getActivity(), anterior.toString(), Toast.LENGTH_SHORT).show();
            play(anterior);
